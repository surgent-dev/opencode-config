...
└── _generated/      # AUTO-GENERATED by convex dev/sync
    ├── api.ts       # Function references for frontend
    ├── server.ts    # Typed query/mutation/action helpers
    └── dataModel.ts # Schema types
```

### Generating `_generated/` (CRITICAL)

The `convex/_generated/` folder is created by running `dev-run` with `syncConvex: true`.
